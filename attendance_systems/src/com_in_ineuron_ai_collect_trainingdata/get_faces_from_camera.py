...
class TrainingDataCollector:
    ...
    def collectImagesFromCamera(self):
        ...
        while faces < max_faces:#start with 0 and maximum faces is 50
            ret, frame = cap.read()#returning the frame and whether it is true or not
            frames += 1

            dtString = str(datetime.now().microsecond)#datetime
            # Get all faces on current frame
            bboxes = self.detector.detect_faces(frame)#we are calling the MTCNN
